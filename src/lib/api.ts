...
export const authApi = {
  ...
  register: async (userData: { name: string; email: string; password: string; role: string; department?: string }) => {
    const res = await axiosClient.post<ApiResponseEnvelope<{ token: string; user: User }>>('/auth/register', userData);
    return res.data;
  },
  getMe: async () => {
  ...
  changePassword: async (currentPassword: string, newPassword: string) => {
    const res = await axiosClient.put<ApiResponseEnvelope<null>>('/auth/change-password', {
      currentPassword,
      newPassword,
    });
    return res.data;
  ...
  logout: async () => {
    const res = await axiosClient.post<ApiResponseEnvelope<null>>('/auth/logout');
    return res.data;
  },
};
