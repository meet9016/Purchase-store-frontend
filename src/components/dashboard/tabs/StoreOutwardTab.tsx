    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-xs">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-[#0F172C]">Store Outward Issues</h3>
      ...
      <Table
        ...
        renderRow={(out) => (
          <tr key={out.id} className="hover:bg-slate-50 transition-colors">
            ...
            <td className="px-4 py-3 text-slate-900 text-xs font-semibold">{out.issuedTo}</td>
            <td className="px-4 py-3 text-slate-800 text-xs font-medium">{out.department}</td>
            <td className="px-4 py-3 text-slate-700 text-xs font-normal max-w-[140px] truncate" title={out.purpose}>{out.purpose}</td>
            ...
            <td className="px-4 py-3 text-slate-800 text-xs font-medium">{out.issuedByName || out.issuerName || '-'}</td>
            <td className="px-4 py-3">
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${
                out.status === 'Issued' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/70' : 'bg-slate-100 text-slate-700 border border-slate-200'
              }`}>
                {out.status || 'Issued'}
              </span>
            </td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-xl w-full p-6 animate-modal-zoom space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1.5">
                <p className="text-xs font-bold text-slate-700">Issue Details</p>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1.5">
                <p className="text-xs font-bold text-slate-700">Project Info</p>
