    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-xs">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-[#0F172C]">Purchase Requests (PR)</h3>
      ...
      <Table
        ...
        itemsPerPage={10}
        renderRow={(pr, idx) => (
          <tr key={`${pr.id || pr.prNumber}-${idx}`} className="hover:bg-slate-50 transition-colors">
            ...
            <td className="px-4 py-3 font-medium text-slate-800 text-xs">{pr.requesterName}</td>
            <td className="px-4 py-3 font-medium text-slate-800 text-xs whitespace-nowrap">{formatDate(pr.requiredDate)}</td>
            ...
            <td className="px-4 py-3">
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${
                pr.priority === 'High' || pr.priority === 'Urgent' ? 'bg-rose-50 text-rose-700 border border-rose-200/70' : 'bg-slate-100 text-slate-700 border border-slate-200'
              }`}>
                {pr.priority}
              </span>
            </td>
            <td className="px-4 py-3">
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold ${
                ...
                pr.status === 'PO Created' ? 'bg-blue-50 text-blue-700 border border-blue-200/70' :
