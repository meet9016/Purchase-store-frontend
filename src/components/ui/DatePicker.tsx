      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl z-55 w-72 p-4 animate-modal-zoom">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            ...
            <span className="font-bold text-sm text-[#0F172C]">
          ...
          {/* Days of Week */}
          <div className="grid grid-cols-7 gap-1 py-2 text-center text-xs font-bold text-slate-400">
            {daysOfWeek.map((day) => (
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-xs">
            ...
            {Array.from({ length: firstDay }).map((_, idx) => (
              <div key={`blank-${idx}`} className="p-1.5" />
            ))}
